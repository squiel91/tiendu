<template>
  <div class="static-selectable-table-snippet table" :class="{'add-spacing': totalPages <= 1 }">
    <div class="row header">
      <div class="col">Incluido</div>
      <div v-for="col in cols" class="col" :style="{ 'grid-column': 'span ' + (col.span || 1) }" :class="{'hide-mobile': col.hideMobile}" :key="col">{{ col.label }}</div>
    </div>
